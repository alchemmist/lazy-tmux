...
func TestMatchesAndRestore(t *testing.T) {
	i := New(t.TempDir())
	if !i.Matches(snapshot.Pane{CurrentCmd: "codex"}) || !i.Matches(snapshot.Pane{RestoreCmd: "codex resume abc"}) {
	...
	if got := i.RestoreCommand(snapshot.Pane{}, map[string]string{metaSessionID: "abc"}); got != "codex resume abc" {
